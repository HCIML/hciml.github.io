...
Hidden or latent variables \\( x_h \\) are parameters which are essential for the model but potentially cannot be observed. This can be missing variables which typically miss at random due to imperfection of data and for a graphical model are simply modelled by an index function \\( \textbf{I}_m \\). 
